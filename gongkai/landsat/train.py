...
from dat.change_detection import ChangeDetection_Landsat_label, ChangeDetection_Landsat_nolabel
import torch
...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--data_name", type=str, default=r"Landsat")
...
class Trainer:
    def __init__(self, args):
        args.log_dir = os.path.join('/home/user/zly/data3/Landsat/pth/gongkai/', 'logs_t0.10', args.data_name,
                                    args.Net_name, args.backbone)
        if not os.path.exists(args.log_dir): os.makedirs(args.log_dir)
        self.writer = SummaryWriter(args.log_dir)
        self.args = args

        trainset_label = ChangeDetection_Landsat_label(root=args.data_root, mode="train")
        trainset_nolabel = ChangeDetection_Landsat_nolabel(root=args.data_root,mode="train")
        valset = ChangeDetection_Landsat_nolabel(root=args.data_root,mode="val")
    ...
    def training(self, epoch):
        ...
        if curr_epoch < 10:
            ...
            class_names_2_label, confidences_2_label, _) in enumerate(tbar):
                ...
                mask1_label = (mask1_label / 10).cuda().long()
                mask2_label = (mask2_label / 10).cuda().long()
            ...
            for i, ((img1_label, img2_label, mask1_label, mask2_label, mask_bn_label, class_names_1_label,
                ...
                loss = loss + 0.01 * (pse_loss_bn + pse_loss_similarity + pse_loss_seg)
    ...
    def validation(self, epoch):
        ...
        if score >= self.previous_best:
            model_path = "/home/user/zly/data3/Landsat/pth/gongkai/t0.10/checkpoints/%s/%s/%s" % \
                         (self.args.data_name, self.args.Net_name, self.args.backbone)
            if not os.path.exists(model_path): os.makedirs(model_path)
            torch.save(self.model.state_dict(),
                       "/home/user/zly/data3/Landsat/pth/gongkai/t0.10/checkpoints/%s/%s/%s/epoch%i_Score%.2f_mIOU%.2f_Sek%.2f_Fscd%.2f_OA%.2f.pth" %
                       (
