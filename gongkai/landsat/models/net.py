...
class ournet(nn.Module):
    ...
    def forward(self, x1, x2,  confidences_1, confidences_2):
        ...
        feature_orig = [torch.cat([feature1[i], feature2[i]], dim=1) for i in range(len(feature1))]

        feature_minus = [(torch.abs(feature1[i] - feature2[i])) for i in range(len(feature1))]

        feature_diff = [F.sigmoid(1 - torch.cosine_similarity(feature1[i], feature2[i], dim=1)).unsqueeze(1) for i in range(len(feature1))]

        feature_text_diff = []
        for i in range(len(feature1_text)):
            feature_text_diff.append(torch.abs(feature1_text[i] - feature2_text[i]))

        feature_fusion = [ self.adjust_conv[i]( torch.cat([feature_orig[i]  * feature_diff[i], feature_minus[i], feature_orig[i]], dim=1)) for i in range(len(feature_orig))]
