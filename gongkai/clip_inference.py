...
if __name__ == '__main__':
    ...
    # Add arguments
    parser.add_argument('--src_path', type=str, default='/home/user/zly/data3/SECOND/sample/t0.10/test',
                        help='Path to the source dataset')
    parser.add_argument('--split', nargs='+', type=str, default=[''], help='Split(s) to use (train, val, test)')
    parser.add_argument('--img_split', nargs='+', type=str, default=['im1', 'im2'], help='Image split(s) to use (A, B)')
    ...
    parser.add_argument('--tag', type=str, default='56_ViTB16', help='Batch size for model execution')
    ...
    for sp in split:
        for isp in img_split:
            ...
            with open(os.path.join(image_folder_path + '_clipcls_{}.json'.format(args.tag)),
                      mode='w') as predictions_file:
